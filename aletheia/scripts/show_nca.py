...
def load_tr_data(dataset_name, splits):
    datasets = [
        {
...
def main():
    X_tr, y_tr = load_tr_data("asvspoof19", splits=["train", "dev"])
    X_te, y_te = load_te_data("asvspoof19", split=["test"])
    knn = KNeighborsClassifier(n_neighbors=5)
    y_pred = knn.fit(X_tr, y_tr).predict(X_te)
